...
function WorkloadChart({ data, selected }: { data: Row[], selected: string[] }) { return <Card><CardHeader><CardTitle>Pay-period burden trend</CardTitle><CardDescription>Federal pay periods only; monthly views removed.</CardDescription></CardHeader><CardContent className="px-2 pt-4 sm:px-6 sm:pt-6"><ChartContainer config={chartConfig} className="aspect-auto h-[360px] w-full"><LineChart data={data}><CartesianGrid vertical={false} /><XAxis dataKey="pp_start" tickLine={false} axisLine={false} minTickGap={32} /><YAxis tickLine={false} axisLine={false} /><ChartTooltip content={<ChartTooltipContent />} />{selected.map((s, i) => <Line key={s} dataKey={s} type="monotone" stroke={colors[i % colors.length]} strokeWidth={2} dot={false} />)}</LineChart></ChartContainer></CardContent></Card> }
function ShiftBars({ data, selected }: { data: Row[], selected: string[] }) { return <Card><CardHeader><CardTitle>Adjusted shift burden by pay period</CardTitle><CardDescription>Raw shifts minus AL credit where entered.</CardDescription></CardHeader><CardContent><ChartContainer config={chartConfig} className="h-[320px] w-full"><BarChart data={data}><CartesianGrid vertical={false} /><XAxis dataKey="pp_start" tickLine={false} axisLine={false} minTickGap={32} /><YAxis tickLine={false} axisLine={false} /><ChartTooltip content={<ChartTooltipContent />} />{selected.map((s, i) => <Bar key={s} dataKey={s} fill={colors[i % colors.length]} radius={4} />)}</BarChart></ChartContainer></CardContent></Card> }
  ...
  return <SidebarProvider style={{ "--sidebar-width": "calc(var(--spacing) * 72)", "--header-height": "calc(var(--spacing) * 12)" } as React.CSSProperties}>
    ...
    <SidebarInset>
      ...
      <div className="flex flex-1 flex-col"><div className="@container/main flex flex-1 flex-col gap-2"><div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
        ...
        <div className="px-4 lg:px-6"><Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground"><b className="text-foreground">{primary}</b> averages <b className="text-foreground">{fmt.format(mine.avg_hours_pp)} h/pay period</b>. Heaviest pay period in range: <b className="text-foreground">{top[0]?.pp ?? "—"}</b> at <b className="text-foreground">{fmt.format(top[0]?.hours ?? 0)} h</b>. Backup shifts count as 8h. Non-TEC/non-backup shifts include 2h cleanup time. AL credit: 3 days = 2 shifts.</p></CardContent></Card></div>
        <div className="px-4 lg:px-6"><Tabs value={tab} onValueChange={setTab}><TabsList className="grid w-full grid-cols-3 lg:grid-cols-6"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="periods">Pay Periods</TabsTrigger><TabsTrigger value="comparison">Comparison</TabsTrigger><TabsTrigger value="outliers">Outliers</TabsTrigger><TabsTrigger value="detail">Detail</TabsTrigger><TabsTrigger value="assumptions">Assumptions</TabsTrigger></TabsList>
          ...
          <TabsContent value="periods" className="mt-4"><div className="overflow-hidden rounded-lg border"><Table><TableHeader className="bg-muted"><TableRow>{["pp", "pp_start", "pp_end", "shifts", "hours", "backups", "AL days", "AL credit", "adjusted burden"].map(c => <TableHead key={c}>{c}</TableHead>)}</TableRow></TableHeader><TableBody>{pp.filter((r: Row) => r.person === primary).sort((a: Row, b: Row) => String(b.pp_start).localeCompare(String(a.pp_start))).map((r: Row) => <TableRow key={r.pp_start}><TableCell>{r.pp}</TableCell><TableCell>{r.pp_start}</TableCell><TableCell>{r.pp_end}</TableCell><TableCell>{fmt.format(Number(r.shifts))}</TableCell><TableCell>{fmt.format(Number(r.hours))}</TableCell><TableCell>{fmt.format(Number(r.backups))}</TableCell><TableCell><input className="w-20 rounded-md border bg-background px-2 py-1" type="number" min="0" step="1" value={alDays[r.pp_start] ?? ""} placeholder="0" onChange={(e) => setAlDays({ ...alDays, [r.pp_start]: Number(e.target.value || 0) })} /></TableCell><TableCell>{fmt.format(Number(r.al_shift_credit))}</TableCell><TableCell>{fmt.format(Number(r.adjusted_shifts))}</TableCell></TableRow>)}</TableBody></Table></div></TabsContent>
          ...
          <TabsContent value="assumptions" className="mt-4"><Card><CardHeader><CardTitle>Assumptions</CardTitle><CardDescription>How the audit interprets the source grid.</CardDescription></CardHeader><CardContent><ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground">{data.assumptions.map((a: string) => <li key={a}>{a}</li>)}<li>Backup shifts count as 8 hours.</li><li>Every non-TEC, non-backup worked shift includes 2 hours of cleanup time.</li><li>Annual leave can be entered per pay period; 3 AL days reduces shift burden by 2 shifts.</li></ul></CardContent></Card></TabsContent>
        </Tabs></div>
      </div></div></div>
    </SidebarInset>
  </SidebarProvider>
