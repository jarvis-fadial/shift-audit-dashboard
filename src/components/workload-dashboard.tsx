...
function alShiftCredit(days: number) { return days }
...
function SectionCards({ mine, peerAvg }: { mine: Row, peerAvg: number | null }) {
  const items = [
    ...
    ["Adjusted Burden", mine.adjusted_shift_burden ?? mine.total_shifts, "AL: 1 day = 1 10h shift", <CalendarDays key="i" />],
...
function ShiftBars({ data, selected }: { data: Row[], selected: string[] }) { return <Card><CardHeader><CardTitle>Adjusted shift burden by pay period</CardTitle><CardDescription>Raw shift count minus AL credit where entered: 1 AL day = 1 10h shift.</CardDescription></CardHeader><CardContent><ChartContainer config={chartConfig} className="h-[320px] w-full"><BarChart data={data}><CartesianGrid vertical={false} /><XAxis dataKey="pp_start" tickLine={false} axisLine={false} minTickGap={32} /><YAxis tickLine={false} axisLine={false} /><ChartTooltip content={<ChartTooltipContent />} />{selected.map((s, i) => <Bar key={s} dataKey={s} fill={colors[i % colors.length]} radius={4} />)}</BarChart></ChartContainer><ChartLegend selected={selected} /></CardContent></Card> }
  ...
  return <SidebarProvider style={{ "--sidebar-width": "calc(var(--spacing) * 72)", "--header-height": "calc(var(--spacing) * 12)" } as React.CSSProperties}>
    ...
    <SidebarInset>
      ...
      <div className="flex flex-1 flex-col"><div className="@container/main flex flex-1 flex-col gap-2"><div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
        ...
        <div className="px-4 lg:px-6"><Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground"><b className="text-foreground">{primary}</b> averages <b className="text-foreground">{fmt.format(mine.avg_hours_pp)} h/pay period</b>. Heaviest pay period in range: <b className="text-foreground">{top[0]?.pp ?? "—"}</b> at <b className="text-foreground">{fmt.format(top[0]?.hours ?? 0)} h</b>. Backup shifts count in shift/back-up totals but add <b className="text-foreground">0 hours</b>. Non-TEC/non-backup shifts include 2h cleanup time. AL credit: 1 day = 1 10h shift.</p></CardContent></Card></div>
        <div className="px-4 lg:px-6"><Card className="border-amber-300/60 bg-amber-50/50 dark:bg-amber-950/20"><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="size-5 text-amber-600" /> Annual leave adjustments</CardTitle><CardDescription>Enter AL directly in the Pay Periods tab. Each pay-period row now has a visible <b>AL days</b> box; values are saved in this browser.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between"><div><b className="text-foreground">Current range:</b> {fmt.format(totalAlDays)} AL days → {fmt.format(totalAlCredit)} shift-credit reduction. Formula: 1 AL day = 1 10h shift.</div><Button variant="outline" onClick={() => setTab("periods")}>Add / edit AL days</Button></CardContent></Card></div>
        <div className="px-4 lg:px-6"><Tabs value={tab} onValueChange={setTab}><TabsList className="grid w-full grid-cols-3 lg:grid-cols-6"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="periods">Pay Periods</TabsTrigger><TabsTrigger value="comparison">Comparison</TabsTrigger><TabsTrigger value="outliers">Outliers</TabsTrigger><TabsTrigger value="detail">Detail</TabsTrigger><TabsTrigger value="assumptions">Assumptions</TabsTrigger></TabsList>
          ...
          <TabsContent value="periods" className="mt-4 space-y-3"><Card><CardContent className="pt-4 text-sm text-muted-foreground"><b className="text-foreground">Where to add AL:</b> use the highlighted <b>AL days</b> column below. Enter the number of annual leave days in that pay period; the dashboard subtracts one 10h shift-equivalent for each AL day.</CardContent></Card><div className="overflow-hidden rounded-lg border"><Table><TableHeader className="bg-muted"><TableRow>{["pp", "pp_start", "pp_end", "shifts", "hours", "backups", "AL days", "AL credit", "adjusted burden"].map(c => <TableHead key={c} className={c.includes("AL") ? "bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-100" : undefined}>{c}</TableHead>)}</TableRow></TableHeader><TableBody>{primaryPp.map((r: Row) => <TableRow key={r.pp_start}><TableCell>{r.pp}</TableCell><TableCell>{r.pp_start}</TableCell><TableCell>{r.pp_end}</TableCell><TableCell>{fmt.format(Number(r.shifts))}</TableCell><TableCell>{fmt.format(Number(r.hours))}</TableCell><TableCell>{fmt.format(Number(r.backups))}</TableCell><TableCell className="bg-amber-50/60 dark:bg-amber-950/20"><input aria-label={`AL days for ${r.pp}`} className="w-24 rounded-md border border-amber-300 bg-background px-2 py-1 font-medium" type="number" min="0" step="1" value={alDays[r.pp_start] ?? ""} placeholder="Add AL" onChange={(e) => updateAl(r.pp_start, Number(e.target.value || 0))} /></TableCell><TableCell>{fmt.format(Number(r.al_shift_credit))}</TableCell><TableCell>{fmt.format(Number(r.adjusted_shifts))}</TableCell></TableRow>)}</TableBody></Table></div></TabsContent>
          ...
          <TabsContent value="assumptions" className="mt-4"><Card><CardHeader><CardTitle>Assumptions</CardTitle><CardDescription>How the audit interprets the source grid.</CardDescription></CardHeader><CardContent><ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground">{data.assumptions.map((a: string) => <li key={a}>{a}</li>)}<li>Backup shifts count as shifts/backups but contribute 0 hours.</li><li>Every non-TEC, non-backup worked shift includes 2 hours of cleanup time.</li><li>Annual leave can be entered per pay period; each AL day reduces burden by one 10h shift-equivalent.</li></ul></CardContent></Card></TabsContent>
        </Tabs></div>
      </div></div></div>
    </SidebarInset>
  </SidebarProvider>
